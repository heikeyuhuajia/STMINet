...
class AmbiguityRefinementModule(nn.Module):
    ...
    def forward(self, t1, t2):
        ...
        sim = self.sim_pool(torch.abs(sim1 - sim2))     # notice sim pool
        ...
        out = self.fusion(torch.cat([attn1, attn2], dim=1)) + dif
        return out
class AmbiguityRefinementModule_mod1(nn.Module):
    
    def __init__(self, in_channel, out_channel, upsize):
        super(AmbiguityRefinementModule_mod1, self).__init__()
        self.c = out_channel
        self.up = upsize
        self.cos = nn.CosineSimilarity(dim=-1)
        self.t1 = nn.Conv2d(in_channel, out_channel, kernel_size=1)
        ...
        self.sim_pool = nn.AvgPool2d(kernel_size=4)
    ...
    def forward(self, t1, t2):
        ...
        branch1 = inception1.contiguous().view(b, c, -1)
        attn1 = nn.Softmax(dim=-1)(torch.bmm(attn, branch1))
        attn1 = torch.bmm(attn.permute(0, 2, 1), attn1).view(b, c, h, w) + dif

        out = self.fusion(torch.cat([attn1], dim=1)) + dif
        return out
